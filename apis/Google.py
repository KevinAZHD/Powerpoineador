...
# Clase para la ventana de configuración de la clave API de Google AI
class GoogleAPIKeyWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        layout.addWidget(QLabel(obtener_traduccion('google_api_key_prompt', self.current_language)))
        ...
        if self.parent and self.parent.google_api_key:
            self.api_input.setText(self.parent.google_api_key)
            ...
            self.copy_btn.clicked.connect(lambda: QApplication.clipboard().setText(self.parent.google_api_key))
    ...
    # Función para validar la clave API con el servidor de Google AI
    def validate_api(self):
        ...
        try:
            ...
            if response.status_code == 200:
                if self.parent:
                    self.parent.set_google_api_key(api_key)
                self.status_label.setText(obtener_traduccion('valid_key', self.current_language))
                self.status_label.setStyleSheet("color: green; qproperty-alignment: AlignCenter;")
